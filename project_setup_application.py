...
class ProjectWindow(QWidget):
    ...
    def debug_project(self, cfg_target, arm_target):
        self.openocd_worker = Debugger(f"./runner/xpack-openocd-0.12.0-7/bin/openocd.exe -f ./runner/xpack-openocd-0.12.0-7/bin/{cfg_target}")
        self.gdb_worker = Debugger(
            f"./runner/xpack-arm-none-eabi-gcc-14.2.1-1.1/bin/arm-none-eabi-gdb.exe "
            f"-ex 'target extended-remote localhost:3333' "
            f"./target/{arm_target}/debug/mikrosdk"
        )

        # Optional: connect outputs to a log window
        self.openocd_worker.output.connect(lambda text: print(f"[OpenOCD] {text}"))
        self.gdb_worker.output.connect(lambda text: print(f"[GDB] {text}"))

        self.openocd_worker.start()

        # Wait a short while to ensure OpenOCD is ready before connecting GDB
        QTimer.singleShot(2000, self.gdb_worker.start)
